...
from chatbot import (
    _THREAD_METADATA, _THREAD_RETRIEVERS, save_message, save_thread, update_thread_title, workflow,retrive_threads,ingest_pdf,create_user,login_user,load_messages,delete_thread,rename_thread,get_thread_title
    )
# from chat_tool import workflow1,retrive_threads
...
#     if st.session_state.get("last_user") != st.session_state.username:
#         st.session_state['chat_threads'] = retrive_threads(st.session_state.username)
...
#             'configurable': {
#                 'thread_id': thread_id,
#                 'user': st.session_state.username   # ✅ ADD THIS
#             }
#         }
#     )

# def load_conversation1(thread_id):
#     return workflow.get_state(config={'configurable' : {'thread_id' : thread_id},'user': st.session_state.username}).values['messages']
def load_conversation(thread_id):
...
#     msg = state.values['messages'][0].content
